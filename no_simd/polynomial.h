...
public:
	...
	void operator*=(const Polynomial<Number> &p) {
		...
		for (unsigned int this_i = 0; this_i < _coef.size(); ++this_i) {
			for (unsigned int p_i = 0; p_i < p._coef.size(); ++p_i) {
	...
	void operator+=(const Polynomial<Number> &p) {
		...
		unsigned int prev_size = _coef.size();
		if (p._coef.size() > _coef.size())
			_coef.resize(p._coef.size());
		for (unsigned int i = prev_size - 1; i < _coef.size(); ++i)
			_coef[i] = 0;

		for (unsigned int i = 0; i < p._coef.size(); ++i) {
			_coef[i] += p._coef[i];
		}
	...
	Polynomial<Number> operator-() const {
		...
		ret.set_mod(_mod);
		for (unsigned int i = 0; i < ret._coef.size(); ++i)
